...
def draw_pred(img, class_id, confidence, x, y, x_plus_w, y_plus_h):
    ...
    cv2.putText(img, label, (x-10,y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.25, color, 2)
...
while cv2.waitKey(1) < 0:
    ...
    for out in outs: 
        ...
        #each detection  has the form like this [center_x center_y width height obj_score class_1_score class_2_score ..]
                ...
                row1 += 1
                row2 += 1
                worksheet.write('A' + str(row1), class_id)
                worksheet.write('B' + str(row2), datetime.datetime.strftime(datetime.datetime.now(), "%Y-%m-%d %H:%M:%S")
